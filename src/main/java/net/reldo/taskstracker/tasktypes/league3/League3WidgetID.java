package net.reldo.taskstracker.tasktypes.league3;

public class League3WidgetID
{
	public static final int LEAGUE_3_TASKS_GROUP_ID = 657;
	public static final int LEAGUE_3_SUMMARY_TAB = 736;

	public static class League3Tasks
	{
		public static final int CONTAINER = 1;
		public static final int CONTAINER_2 = 1;
		public static final int TASK_BAR = 3;
		public static final int HORIZONTAL_SEPARATOR = 4;
		public static final int VERTICAL_SEPARATOR = 5;
		public static final int TASK_LIST_CONTAINER = 6;
		public static final int TASK_LIST_SCROLLBAR = 7;
		public static final int TASK_LIST_VIEWPORT = 8;
		public static final int TASK_LIST = 9;
		public static final int TASK_LIST_TITLES = 10;
		public static final int TASK_LIST_MONSTER_OR_TYPE = 11;
		public static final int TASK_LIST_TYPE_OR_MONSTER = 12;
		public static final int TASK_LIST_DESCRIPTION = 13;
		public static final int TASK_LIST_EXPAND_BUTTONS = 14;
		public static final int TASK_LIST_TIER_ICONS = 15;
		public static final int TASK_LIST_NO_FILTER_RESULTS = 16;
		public static final int FILTERS_CONTAINER = 18;
		public static final int FILTERS_LABEL = 19;
		public static final int FILTER_TIER = 20;
		public static final int FILTER_TYPE = 21;
		public static final int FILTER_AREA = 22;
		public static final int FILTER_SKILL = 23;
		public static final int FILTER_COMPLETED = 24;
		public static final int CLOSE_BUTTON = 25;
		public static final int HAMBURGER_BUTTON = 26;
		public static final int UNKNOWN_OVERLAY_OR_BG = 26;
		public static final int HAMBURGER_DROPDOWN = 28;
		public static final int FILTER_DROPDOWN_CONTAINER = 30;
		public static final int FILTER_DROPDOWN = 31;
		public static final int FILTER_DROPDOWN_BACKGROUND = 32;
		public static final int FILTER_DROPDOWN_TIER_VALUES = 33;
		public static final int FILTER_DROPDOWN_TYPE_VALUES = 34;
		public static final int FILTER_DROPDOWN_AREA_VALUES = 35;
		public static final int FILTER_DROPDOWN_SKILL_VALUES = 36;
		public static final int FILTER_DROPDOWN_COMPLETED_VALUES = 37;


		public static final int RESIZABLE_CONTAINER = 38; //maybe?
	}

	public static class League3SummaryTab
	{
		public static final int CONTAINER = 1;
		public static final int TITLE = 2;
		public static final int CONTENT = 3;
		public static final int CONTENT_BACKGROUND = 4;
		public static final int CONTENT_CONTAINER = 5;
		public static final int TIER_TEXT = 6;
		public static final int PROGRESS_BAR_BORDER = 7;
		public static final int PROGRESS_BAR = 8;
		public static final int PROGRESS_BAR_BACKGROUND = 9;
		public static final int PROGRESS_BAR_PROGRESS = 10;
		public static final int FRAGMENT_ICON = 11;
		public static final int TASKS_COMPLETED_LABEL = 12;
		public static final int TASKS_COMPLETED_VALUE = 13;
		public static final int LEAGUE_POINTS_LABEL = 14;
		public static final int LEAGUE_POINTS_VALUE = 15;
		public static final int SAGE_RENOWN_LABEL = 16;
		public static final int SAGE_RENOWN_VALUE = 17;
		public static final int NAVIGATION_BUTTONS = 18;
	}
}
